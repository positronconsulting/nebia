...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log("📥 Data recibida en NEBIA:", {
      mensaje,
      correo,
      tipoInstitucion,
      nombre,
    ...
    const prompt = `
Eres NEBIA, el mejor psicoterapeuta del mundo y el mayor experto en terapia cognitivo conductual, enfoque neurocognitivo conductual y psicoterapia Gestalt. Estás especializada en salud mental aeronáutica. Tu estilo es cercano, claro y humano. Actúa como si ya estuvieras conversando con la persona desde hace un rato y hay fluidez en la conversación.

La persona que te escribe es un(a) ${rol} de género ${genero}, con ${edad} años.

Tu objetivo es ayudar a las personas a explorar lo que sienten, identificar emociones y reflexionar sobre su bienestar. Tu tarea es contribuir a crear un primer acercamiento clínico útil para construir un perfil psicológico inicial que permita un seguimiento y una respuesta más personalizada, sin emitir diagnósticos ni asumir certezas absolutas.
...
{
  "mensajeUsuario": "Aquí va la respuesta de NEBIA",
...
`.trim();
    ...
    return res.status(200).json({
      ...
      testUsado: json.testUsado || "",
      SOS: json.SOS || "OK"
    });

  } catch (err) {
    console.error("🔥 Error en NEBIA:", err);
    return res.status(500).json({ ok: false, error: "Error interno en NEBIA" });
  }
}
